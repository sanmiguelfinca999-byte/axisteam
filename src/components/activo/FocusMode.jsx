...
export default function FocusMode() {
  const { currentUser, tasks, completarTarea, actualizarProgreso } = useNEXUS()
  const misTareas = useMemo(() => {
    if (!currentUser || currentUser.role === 'CORONEL') return []
    ...
    <div className="max-w-2xl mx-auto p-4 space-y-4">
      ...
      {misTareas.length === 0 && (
        <div className="text-center py-16 text-nexus-muted">
          ...
          <p className="font-mono">Sin misiones asignadas. En espera de ordenes.</p>
        </div>
      )}
    </div>
  )
